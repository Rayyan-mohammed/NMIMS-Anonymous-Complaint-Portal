...
<head>
    ...
    <title>Program Chair Dashboard - STME</title>
...
<body>
    ...
    <div class="container">
        <h1>Program Chair Dashboard - STME</h1>

        <!-- Program Chair Complaints Section -->
        <div class="section" id="programChairComplaints">
            <h2 class="section-title">Program Chair Complaints</h2>
            <div class="table-container">
                <?php
                $host = "localhost";
                $user = "root";
                $password = "";
                $database = "stme"; // Change this for other schools (sbm, sol, sptm)
                ...
                $sql = "SELECT reference_number, complaint_details, status, submitted_at, comment FROM program_chair_complaints";
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td><textarea class='comment-textarea' data-ref='" . $row['reference_number'] . "' data-table='program_chair_complaints' data-database='$database'>" . htmlspecialchars($row['comment'] ?? '') . "</textarea></td>";
                        echo "<td>" . $row['submitted_at'] . "</td>";
                        echo "<td>";
                        echo "<select onchange=\"updateStatus('program_chair_complaints', '" . $row['reference_number'] . "', this.value, '$database')\">";
